...
class BanditStrategy(object):
    ...
    def _softmax(self):
        ...
        if self.trials.min() == 0:
            return np.argmin(self.trials)
        pmf= np.exp(1)**((self.wins / self.trials)/tau)
        size = self.n_bandits
        #print 'size', size, 'pmf', pmf
        #import pdb; pdb.set_trace()
        x = np.random.choice(size,p=(pmf/sum(pmf)))
        return x
    ...
    def _ucb1(self):
        ...
        if self.trials.min() == 0:
            ...
            return np.argmax(self.wins / self.trials + (2*np.log(self.N)/self.trials)**0.5)
    ...
    def _bayesian_bandit(self):
        ...
        for a,b in zip(alpha,beta):
            samps.append(np.random.beta(a,b,size=1))
